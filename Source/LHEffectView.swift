...
open class LHEffectView: UIView {
    ...
    open override var frame: CGRect {
        willSet {
            shapeLayer.frame = CGRect(x: 0, y: 0, width: newValue.width, height: newValue.height)
            gradientLayer.frame = CGRect(x: 0, y: 0, width: newValue.width, height: newValue.height)
        }
        didSet {
...
open class LHCornerView: UIView {
    ...
    internal func updateGradient() {
        ...
        if let gColors = colors, gColors.count > 0 {
            if gColors.count == 1 {
                gradientLayer.colors = [gColors.first!.cgColor, gColors.first!.cgColor]
            } else {
                gradientLayer.colors = colors?.map({ (gColor) -> CGColor in
                    return gColor.cgColor
                })
    ...
    override open func draw(_ rect: CGRect) {
        ...
        if #available(iOS 11.0, *) {
            ...
            let roundPath = self.makePath(self.borderWidth)
            roundPath.lineWidth = self.borderWidth * 2.0
            self.borderColor?.setStroke()
            roundPath.stroke()
...
@IBDesignable
open class LHShadowView: UIView {
    // MARK: Shadow ========================================================
    @IBInspectable
    open var shadowColor: UIColor? = UIColor.clear {
        didSet {
    ...
    internal func setUpdateNeeds() { setNeedsDisplay() }
